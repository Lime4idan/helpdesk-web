...
function rotuloStatus(status) {
  return ({ 'Aberto': 'Open', 'Em Atendimento': 'In progress', 'Concluído': 'Resolved' })[status] || status;
}

function rotuloPerfil(tipo) {
  return tipo === 'tecnico' ? 'Technician' : 'Customer';
}
...
function renderizarLista(filtro = 'Todos') {
  ...
  if (!chamados.length) {
    lista.innerHTML = '<div class="painel vazio">No tickets found.</div>';
    return;
  }
  lista.innerHTML = chamados.map(chamado => `
    <article class="card shadow-sm cartao">
      <div class="cartao-topo"><span class="status ${classeStatus(chamado.status)}">${escaparHtml(rotuloStatus(chamado.status))}</span><small>#${chamado.id}</small></div>
      <h2>${escaparHtml(chamado.titulo)}</h2>
      <p>${escaparHtml(chamado.descricao).slice(0, 140)}</p>
      <p class="muted">Customer: ${escaparHtml(chamado.cliente_nome)}<br>Technician: ${escaparHtml(chamado.tecnico_nome || 'Unassigned')}</p>
      <a class="btn btn-outline-primary botao secundario" href="chamado.html?id=${chamado.id}">View details</a>
    </article>`).join('');
}
...
if (listaElemento) {
  ...
  if (usuario.tipo === 'tecnico') {
    ...
    document.getElementById('descricao-lista').textContent = 'Review, claim, and update support requests.';
  }
  ...
  document.getElementById('form-chamado').addEventListener('submit', async (evento) => {
    ...
    try {
      ...
      mostrarMensagem('Ticket opened successfully.', 'sucesso');
      await carregarLista();
...
function renderizarDetalhes(chamado) {
  ...
  const controleTecnico = usuario.tipo === 'tecnico' ? `
    <form id="form-status" class="controle-status"><label for="status">Change status and claim ticket</label><div class="acoes"><select id="status"><option value="Aberto" ${chamado.status === 'Aberto' ? 'selected' : ''}>Open</option><option value="Em Atendimento" ${chamado.status === 'Em Atendimento' ? 'selected' : ''}>In progress</option><option value="Concluído" ${chamado.status === 'Concluído' ? 'selected' : ''}>Resolved</option></select><button>Save status</button></div></form>` : '';

  area.innerHTML = `<div class="cartao-topo"><span class="status ${classeStatus(chamado.status)}">${escaparHtml(rotuloStatus(chamado.status))}</span><small>Ticket #${chamado.id}</small></div><h1>${escaparHtml(chamado.titulo)}</h1><p class="descricao">${escaparHtml(chamado.descricao)}</p><div class="metadados"><span><strong>Customer</strong>${escaparHtml(chamado.cliente_nome)}</span><span><strong>Technician</strong>${escaparHtml(chamado.tecnico_nome || 'Unassigned')}</span><span><strong>Updated</strong>${dataFormatada(chamado.atualizado_em)}</span></div>${controleTecnico}<div class="acoes">${editar ? '<button id="botao-editar" class="secundario">Edit</button>' : ''}${podeExcluir ? '<button id="botao-excluir" class="perigo">Delete ticket</button>' : ''}<a class="botao secundario" href="chamados.html">Back</a></div>`;
...
async function alterarStatus(evento) {
  ...
  try {
    ...
    mostrarMensagem('Status updated.', 'sucesso');
  } catch (erro) { mostrarMensagem(erro.message); }
}
...
async function carregarComentarios(id) {
  ...
  lista.innerHTML = comentarios.length ? comentarios.map(item => `<article class="comentario"><div><strong>${escaparHtml(item.usuario_nome)}</strong><span>${escaparHtml(rotuloPerfil(item.usuario_tipo))}</span></div><p>${escaparHtml(item.comentario)}</p><small>${dataFormatada(item.criado_em)}</small></article>`).join('') : '<p class="muted">No comments yet.</p>';
}

async function carregarDetalhes() {
  const id = new URLSearchParams(window.location.search).get('id');
  if (!id) return mostrarMensagem('Ticket ID was not provided.');
  try {
...
if (detalhesElemento) {
  ...
  document.getElementById('form-edicao').addEventListener('submit', async (evento) => {
    ...
    try {
      ...
      renderizarDetalhes(chamadoAtual);
      mostrarMensagem('Ticket updated.', 'sucesso');
    } catch (erro) { mostrarMensagem([erro.message, ...(erro.detalhes || [])].join(' ')); }
  });
